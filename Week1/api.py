...
# now we will be calling from rag_pipelineess
import os
from fastapi import FastAPI, HTTPException
from pydantic import BaseModel
from dotenv import load_dotenv

# will import things from the sared pipeline
...
# will loadanya bjojyam the pipeline at the startup
PDF_PATH = "Paper/Detection_of_Fake_Accounts_on_Social_Media_Using_Multimodal_Data_With_Deep_Learning.pdf"
llm=load_pipeline(PDF_PATH)
...
def ask_question(request: AskRequest):
    ...
    try:
        ...
        best_chunks=rerank(request.question, candidates, top_k=5)
        
        context="\n\n".join([chunk.page_content for chunk in best_chunks])
        prompt=f"""Answer the question using only the context below.
If answer is not in the context, simply say, "I don't know!".
